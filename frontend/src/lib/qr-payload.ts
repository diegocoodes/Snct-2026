...
/** QR do stand para o avaliador escanear. */
export function buildStandQrPayload(qrCodeHash: string, origin?: string) {
  const hash = qrCodeHash.trim();
  if (!hash) return "";
  ...
  return `${base}/avaliador/avaliar?stand=${encodeURIComponent(hash)}`;
}
