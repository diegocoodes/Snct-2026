...
async function readStore(
  ...
  const emptyUsers = Promise.resolve({
    rows: [] as Array<
      {
        id: number;
        role_id: number;
...
export async function readSnctStore(options?: {
  includeUsers?: boolean;
}): Promise<SnctStore> {
  await ensureDomainSeeded();
  return readStore(undefined, {
    includeUsers: options?.includeUsers === true,
  });
}

export async function readPublicSnctStore(): Promise<SnctStore> {
  const store = await readSnctStore({ includeUsers: false });
  return {
    ...store,
    partners: store.partners.filter((partner) => !partner.hidden),
  };
}
...
export async function updateSnctStore<T>(
  ...
  return transaction(async (client) => {
    try {
      ...
      const store = await readStore(client, { includeUsers: true });
