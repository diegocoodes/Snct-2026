...
export async function saveUsuarioFoto(userId: string, buffer: Buffer) {
...
export async function registrarUsuario(
  ...
  // Visitante acessa a credencial pelo CPF — sem login/senha.
  const senhaFinal =
    rolePermitida === "VISITANTE"
      ? `V!${randomBytes(24).toString("base64url")}`
      : senha;
  if (rolePermitida !== "VISITANTE" && senhaFinal.length < 12) {
    return {
      ok: false,
      status: 400,
  ...
  try {
    if (dados.fotoBuffer && dados.fotoBuffer.byteLength > 0) {
      fotoPath = await saveUsuarioFoto(userId, dados.fotoBuffer);
