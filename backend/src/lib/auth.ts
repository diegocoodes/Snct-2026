...
export async function createSession(usuarioId: string) {
  ...
  await query(
    `INSERT INTO sessoes (id, usuario_id, token_hash, expires_at)
     VALUES ($1, $2, $3, $4)`,
    [id, usuarioId, hashToken(token), expiresAt],
  );
...
export async function loginWithPassword(email: string, password: string) {
  ...
  const valid = await verifyPassword(row.senha_hash, password);
  if (!valid) {
    return { ok: false as const, error: "Credenciais inválidas." };
  }
  if (row.role_codigo === "VISITANTE") {
    return {
      ok: false as const,
      error:
        "Visitante não usa login. Consulte o QR Code pelo CPF no menu Visitante.",
    };
  }
  if (row.role_codigo === "ALUNO") {
    return {
      ok: false as const,
      error:
        "Aluno não usa login. O QR Code de check-in é gerenciado pelo professor.",
    };
  }
  const session = await createSession(toId(row.id));
  return {
    ok: true as const,
    user: mapUsuarioRow(row),
    session,
  };
}
